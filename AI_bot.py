...
def playerBlackMove(obj_board, cords, movement_array):
    ...
    # time.sleep(2)
    minimizer = minimax(obj_board, 3, False)
    print(f"the best score black can achieve at this position is: {minimizer}")
    ...
    print(f"{In_row}, {In_col}")
    print(f"{Fn_row}, {Fn_col}")
    #print(f"Static Evaluation of board is {staticEval(obj_board)}")
...
def minimax(obj_board, depth, player):
    ...
    if(depth == 0):
        #print(f"static eval of lowest depth is {staticEval(obj_board)}")
        return staticEval(obj_board)


    if (player == True):
        maxEval = -math.inf
        validMovesArray = allValidMovesWhite(obj_board)
        for node in validMovesArray:
            stEval = minimax(node.obj_board, depth-1, False)
            if(maxEval < stEval):
                maxEval = stEval
    ...
    if (player == False):
        minEval = (math.inf)
        validMovesArray = allValidMovesBlack(obj_board)
        for node in validMovesArray:

            stEval = minimax(node.obj_board, depth-1, True)
            if (minEval > stEval):
                print("reached")
                print(f"{node.In_row}, {node.In_col}")
                print(f"{node.Fn_row}, {node.Fn_col}")
                minEval = stEval
                AI_In_col = node.In_col
                AI_In_row = node.In_row
                AI_Fn_col = node.Fn_col
                AI_Fn_row = node.Fn_row
...
while Run_window == True:
    ...
    if piece_clicked == True:  # shows user what they click
        pg.draw.rect(screen, (246, 255, 120), pg.Rect(cord_system[movement_array[-2]][movement_array[-1]][0],
                                                      cord_system[movement_array[-2]][movement_array[-1]][1], space_dim,
                                                      space_dim))
